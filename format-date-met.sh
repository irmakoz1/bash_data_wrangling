...
# Output files
FORMATTED_FILE1="formatted_filemet.csv"

awk -F, 'BEGIN {OFS=","} 
NR==1 {print; next}  # Print the header without modification
{
    date = $1  # Assuming the date is in the first column
    ...
    if (date ~ /^[0-9]{2}\/[0-9]{2}\/[0-9]{4}$/) {
        split(date, parts, "/")
        $1 = parts[3] "-" parts[1] "-" parts[2]
    }
    # Handle DD-MM-YYYY format
    else if (date ~ /^[0-9]{2}-[0-9]{2}-[0-9]{4}$/) {
        split(date, parts, "-")
        $1 = parts[3] "-" parts[2] "-" parts[1]
    }
    # Handle DD/MM/YY format (European with 2-digit year)
        ...
        $1 = year "-" parts[2] "-" parts[1]  # Convert to YYYY-MM-DD
    }
...
rm output_met.csv formatted_datasetmet.csv 